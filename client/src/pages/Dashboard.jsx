...
const Dashboard = () => {
  ...
  const [selectedSyllabus, setSelectedSyllabus] = useState(null); // For View
  const [syllabusToEdit, setSyllabusToEdit] = useState(null); // For Edit
  ...
  const handleEdit = (e, item) => {
    e.stopPropagation(); // Prevent opening View Modal
    setSyllabusToEdit(item); // Set item to edit
    setIsCreateModalOpen(true); // Open the Create Modal (which now handles edit)
  };

  const handleCreateNew = () => {
    setSyllabusToEdit(null); // Ensure we are in Create mode, not Edit mode
    ...
    <div className="flex h-screen bg-slate-50 w-full">
      ...
      {isCreateModalOpen && (
        <CreateModal
          ...
          syllabusToEdit={syllabusToEdit} // Pass the item to edit
        />
      )}
